...
void TelemetryStore::ensureComparisonAlignment() const {
    ...
    comparisonAlignmentTime_ = speedLandmarkAlignment(*primary, *compare);
    ...
    if (!speedLandmarks) {
        ...
        comparisonAlignmentTime_.resize(qsizetype(primary->time.size()));
        for (size_t i = 0; i < primary->time.size(); ++i) {
            ...
            comparisonAlignmentTime_[qsizetype(i)] = timeAtProgress(progress);
        }
    }
    ...
    if (gpsAvailable) {
        ...
        for (size_t i = 0; i < primary->time.size(); i += anchorStep) {
            ...
            const double baseTime = comparisonAlignmentTime_[qsizetype(i)];
            ...
            if (!anchors.empty() &&
                compare->time[best] <= comparisonAlignmentTime_[qsizetype(
                                           anchors.back().primaryIndex)] +
                                           anchors.back().correction)
                continue;
            anchors.push_back({i, compare->time[best] -
                                      comparisonAlignmentTime_[qsizetype(i)]});
        }
    }
    ...
    if (gpsDistributed) {
        ...
        for (size_t i = 0; i < primary->time.size(); ++i) {
            ...
            comparisonAlignmentTime_[qsizetype(i)] =
                std::clamp(comparisonAlignmentTime_[qsizetype(i)] + correction,
                           compare->time.front(), compare->time.back());
            if (i > 0)
                comparisonAlignmentTime_[qsizetype(i)] =
                    std::max(comparisonAlignmentTime_[qsizetype(i)],
                             comparisonAlignmentTime_[qsizetype(i - 1)]);
        }
        comparisonGpsAnchors_ = gpsAnchorCount;
        comparisonAlignmentBasis_ =
            speedLandmarks ? QStringLiteral("GPS anchored · speed landmarks")
                           : QStringLiteral("GPS anchored · wheel/GPS speed");
    }

    comparisonAlignmentFraction_.resize(comparisonAlignmentTime_.size());
    size_t high = 1;
    for (qsizetype i = 0; i < comparisonAlignmentTime_.size(); ++i) {
        const double time = comparisonAlignmentTime_[i];
        while (high < compare->time.size() && compare->time[high] < time)
            ++high;
        if (high >= compare->time.size()) {
            comparisonAlignmentFraction_[i] = 1.0;
        } else if (time <= compare->time.front()) {
            comparisonAlignmentFraction_[i] = 0.0;
        } else {
            const size_t low = high - 1;
            const double span = compare->time[high] - compare->time[low];
            const double local =
                span > 0.0 ? (time - compare->time[low]) / span : 0.0;
            comparisonAlignmentFraction_[i] =
...
QString TelemetryStore::comparisonAlignmentConfidence() const {
    ensureComparisonAlignment();
    if (comparisonAlignmentBasis_.isEmpty()) return QStringLiteral("NONE");
    if (comparisonGpsAnchors_ >= 8) return QStringLiteral("HIGH");
    if (comparisonAlignmentBasis_.contains(QStringLiteral("speed landmarks")) ||
        comparisonAlignmentBasis_ == QStringLiteral("validated lap distance"))
