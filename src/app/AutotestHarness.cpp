...
bool omatrack::autotest::install(QQmlApplicationEngine& engine,
    ...
    QObject::
        connect(startTimer, &QTimer::timeout, &engine,
                [startTimer, &store, &engine, shotPath, startupVideoPath,
                 ...
                 autotestCornerEdit, autotestDualVideo, autotestLapLoading,
                 sequentialVideoReady]() {
                    if (store.loading() || store.lapLoading() || !store.ready())
                        return;
                    startTimer->stop();
                                ...
                                if (best >= 0) {
                                    ...
                                    if (autotestLapLoading) {
                                        ...
                                        const bool loadingReady =
                                            ...
                                            indicator->property("visible")
                                                .toBool() &&
                                            indicator->property("running")
                                                .toBool() &&
                                            window;
                                        const QImage image =
                                            window ? window->grabWindow()
                                                   : QImage();
                                        const bool saved = !image.isNull() &&
                                                           image.save(shotPath);
                                        qWarning()
                                            << "AUTOTEST lap loading:"
                                            << loadingReady << "saved:" << saved
                                            << image.size();
                                        qApp->exit(loadingReady && saved ? 0
                                                                         : 1);
                                        return;
                                    }
                                    ...
                                    if (autotestCompare) {
                                        for (const QVariant& otherGroupValue :
                                             groups) {
                                    ...
                                    if (autotestSelection &&
                                        ...
                                        if (trace && trace->width() > 0 &&
                                            ...
                                            QCoreApplication::sendEvent(trace,
                                                                        &begin);
                                            QHoverEvent move(
                                                QEvent::HoverMove, finish,
                                                finish, start, Qt::NoModifier);
                                            QCoreApplication::sendEvent(trace,
                                                                        &move);
                                            QMouseEvent commit(
                                                QEvent::MouseButtonPress,
                                    ...
                                    if (autotestZoom &&
                                        ...
                                        if (trace && trace->width() > 0 &&
                                            ...
                                            QGuiApplication::sendEvent(trace,
                                                                       &wheel);
                                    ...
                                    QTimer::
                                        singleShot(autotestDualVideo ? 7000
                                                                     : 2500,
                                                   &engine,
                                                   [&store, &engine, shotPath,
                                                    startupVideoPath,
                                                    autotestWindows,
                                                    autotestRename,
                                                    autotestBrakeSync, autotestCornerEdit, autotestZoom, autotestDualVideo, sequentialVideoReady]() {
                                                       QList<QQuickWindow*>
                                                           windows;
                                                       for (
                                                           QObject* root :
