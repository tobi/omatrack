...
fn recording() -> Recording {
    Recording::synthetic(
        "",
        vec![
            RawChannel::synthetic(
                "Speed",
                "km/h",
                2.0,
                2.0,
                vec![100.0, 120.0, 140.0, 160.0, 180.0],
            ),
            RawChannel::synthetic(
                "RPM",
                "rpm",
                2.0,
                2.0,
                vec![6000.0, 6500.0, 7000.0, 7500.0, 8000.0],
            ),
        ],
    )
}
...
fn source_channels_resample_through_the_source_clock() {
    ...
    assert_eq!(
        catalog.iter().map(|c| c.key.as_str()).collect::<Vec<_>>(),
        ["raw:Speed", "raw:RPM"]
    );
    assert!(catalog.iter().all(|c| !c.default_visible));
    let group = SourceChannels
        .resample(&rec, &lap, &["raw:RPM".into()])
        .unwrap();
    ...
    assert!(
        SourceChannels
            .resample(&rec, &lap, &["raw:Missing".into()])
            .is_err()
    );
}
