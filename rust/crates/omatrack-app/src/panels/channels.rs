//! Channels: which channels the trace workspace shows, and how.
//!
//! This wave shows an honest summary; channel styling replaces the body.

use gpui_kit::{
    Context, FocusHandle, InteractiveElement as _, IntoElement, ParentElement as _, Render,
    Styled as _, Subscription, TestSupportExt as _, Window,
};

use crate::panels::{PanelKind, analysis_body, simple_panel};
use crate::state::AppState;

pub struct ChannelsPanel {
    app: AppState,
    focus_handle: FocusHandle,
    _subscriptions: Vec<Subscription>,
}

impl ChannelsPanel {
    pub fn new(app: AppState, cx: &mut Context<Self>) -> Self {
        let subscriptions = vec![cx.observe(&app.session, |_, _, cx| cx.notify())];
        Self {
            app,
            focus_handle: cx.focus_handle(),
            _subscriptions: subscriptions,
        }
    }
}

simple_panel!(ChannelsPanel, PanelKind::Channels);

impl Render for ChannelsPanel {
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        gpui_kit::div()
            .id("channels-panel")
            .test_support()
            .track_focus(&self.focus_handle)
            .size_full()
            .child(analysis_body(
                "channels-summary",
                &self.app,
                cx,
                |analysis| {
                    let channels: usize = analysis
                        .primary()
                        .overlays()
                        .iter()
                        .map(|group| group.channels.len())
                        .sum();
                    format!("{channels} channels available for the primary lap").into()
                },
            ))
    }
}

/// This panel's app-wide setup: its dock registration (saved layouts
/// rebuild it by name) and, when it has them, its own actions and key
/// bindings. Called once from [`crate::panels::init`].
pub fn init(cx: &mut gpui_kit::App) {
    crate::panels::register(PanelKind::Channels, cx);
}
