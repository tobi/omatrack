...
impl TraceView {
    ...
    pub fn set_mode(&mut self, mode: TraceViewMode, cx: &mut Context<Self>) {
        if mode == self.mode {
            return;
        }
        self.mode = mode;
    ...
    fn ensure_spread(&mut self, cx: &mut Context<Self>) {
    ...
    fn start_load(&mut self, primary: LoadedLap, cx: &mut Context<Self>) {
